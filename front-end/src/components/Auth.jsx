...
const Auth = ({ type }) => {
    ...
    const handleSubmit = async (values, { resetForm }) => {
        ...
        try {
            console.log('Submitting values:', values); // Debugging: Log values before sending

            const response = await axios.post(url, values);
            console.log('Response:', response); // Debugging: Log response data
            ...
            if (type === 'login') {
                setTimeout(() => navigate('/shortenForm'), 1000); // Navigate to dashboard after 2 seconds
            } else {
                setTimeout(() => navigate('/'), 1000); // Navigate to login after 2 seconds
            }
        } catch (error) {
            console.error('Login or registration error:', error); // Debugging: Log detailed error
            setSnackbar({ open: true, message: error.response.data.msg || 'An error occurred', severity: 'error' });
        }
    };
    ...
    return (
        <ThemeProvider theme={defaultTheme}>
            <Grid component={Paper} elevation={3} container sx={{ width: "740px", maxWidth: '100%', borderRadius: '0px' }} justifyContent="center">
                <CssBaseline />
                <Grid
                    item
                    xs={false}
